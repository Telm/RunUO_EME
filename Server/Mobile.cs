...
namespace Server
{
	...
	public class Mobile : IEntity, IHued, IComparable<Mobile>, ISerializable, ISpawnable
	{
        ...
        {
            //Mobile from = this;
            //if (from.m_ADShieldTimer != null)
            //    from.m_ADShieldTimer.Stop();

            //from.m_ADShieldTimer = null;
            //return;
        }
...
private int m_ActualLanguage;
		...
		public int ActualLanguage{
			get{
				if(! Server.Language.ContainsLanguage(m_ActualLanguage)){
					m_ActualLanguage = Server.Language.CommonLanguageIndex;	
				}
			...
			set{
				...
				if(value == 0)
					m_ActualLanguage = Server.Language.CommonLanguageIndex;
				if(Server.Language.Languages[value] == null)
					return;
				else
		...
		public virtual void OnAosSingleClick( Mobile from )
		{
			...
			if( opl.Header > 0 )
			{
                ...
                //imiona
                string name = this.GetNameUseBy(from);
		...
		public virtual int GetLanguageHue(int languageIndex, int actualHue, MessageType type){
		...
		public virtual void DoSpeech( string text, int[] keywords, MessageType type, int hue )
		{
			...
			//m_SpeechHue = langHueue;
			//m_WhisperHue = langHueue;
			//m_YellHue = langHueue;
			switch( type )
			{
			...
			if(type != MessageType.Emote){
				text = Server.Language.EncodeSpeech(this, text, m_ActualLanguage);
			}
			SpeechEventArgs regularArgs = new SpeechEventArgs( this, text, type, hue, keywords );
            
			EventSink.InvokeSpeech( regularArgs );
			this.Region.OnSpeech( regularArgs );
			OnSaid( regularArgs );

			if( regularArgs.Blocked )
				return;

			text = regularArgs.Speech;
			...
			if( MutateSpeech( hears, ref mutatedText, ref mutateContext ) ){
				...
				for( int i = 0; i < hears.Count; ++i )
				{
					...
					if( mutatedArgs == null || !CheckHearsMutatedSpeech( heard, mutateContext ) )
					{
						...
						if(type != MessageType.Emote && !heard.Equals(this)){
							newText = Server.Language.EncodeSpeech(this, newText, m_ActualLanguage);
						}
						
						SpeechEventArgs regArgs = new SpeechEventArgs( this, newText, type, GetLanguageHue(m_ActualLanguage,hue, type), keywords );
						...
						{
                            //system imion
                            string name = this.GetNameUseBy(heard);
                            if (this.m_Serial == heard.m_Serial)
                            {
                                if (this.m_AccessLevel >= AccessLevel.GameMaster)
                                {
                                    Console.WriteLine("mowi do siebie gm");
                                    regp = Packet.Acquire(new UnicodeMessage(m_Serial, Body, type, GetLanguageHue(m_ActualLanguage, hue, type), 3, m_Language, name, NEtext));
                                }
                                else
                                {
                                    Console.WriteLine("mowi do siebie player ?");
                                    regp = Packet.Acquire(new UnicodeMessage(m_Serial, Body, type, GetLanguageHue(m_ActualLanguage, hue, type), 3, m_Language, name, newText));
                                }
                            }
                            else
                            {
                                if (/*this.m_AccessLevel >= AccessLevel.GameMaster ||*/  heard.m_AccessLevel >= AccessLevel.GameMaster)
                                {
                                    Console.WriteLine("slyszy gm");
                                    regp = Packet.Acquire(new UnicodeMessage(m_Serial, Body, type, GetLanguageHue(m_ActualLanguage, hue, type), 3, m_Language, name, NEtext));
                                }
                                else
                                {
                                    Console.WriteLine("slyszy ktos kto nie jest gmem?");
                                    regp = Packet.Acquire(new UnicodeMessage(m_Serial, Body, type, GetLanguageHue(m_ActualLanguage, hue, type), 3, m_Language, name, newText));
                                }
                                //Console.WriteLine("mowie do kogos?!");
                                //regp = Packet.Acquire(new UnicodeMessage(m_Serial, Body, type, GetLanguageHue(m_ActualLanguage,hue), 3, m_Language, name, newText));
                            }
							//if( regp == null )
					...
					{
						if(type != MessageType.Emote && !heard.Equals(this)){
							newText = Server.Language.EncodeSpeech(heard, mutatedText, m_ActualLanguage);
						}
						else
							newText = mutatedText;
						mutatedArgs.Speech = newText;
						...
						{
							if( mutp == null )
								mutp = Packet.Acquire( new UnicodeMessage( m_Serial, Body, type, GetLanguageHue(m_ActualLanguage,hue, type), 3, m_Language, Name, newText ) );
...
SpeechEventArgs regArgs = new SpeechEventArgs( this, text, type, hue, keywords );
					...
					if( obj is Mobile )
					{
						...
						if( mutatedArgs == null || !CheckHearsMutatedSpeech( heard, mutateContext ) ){
	if(type != MessageType.Emote){
							newText = Server.Language.EncodeSpeech(heard, text, m_ActualLanguage);	
							}
						
							regArgs.Speech = newText;
							heard.OnSpeech( regArgs );
						}
						else{
							if(type != MessageType.Emote)
				...
				case 0:
					{
						...
						m_ActualLanguage = reader.ReadInt();
		...
		public void PrivateOverheadMessage( MessageType type, int hue, int number, string args, NetState state )
		{
			if( state == null )
				return;
            //if (state is Mobile)
			state.Send( new MessageLocalized( m_Serial, Body, type, hue, 3, number, GetNameUseBy(state.Mobile), args ) );
            //else
