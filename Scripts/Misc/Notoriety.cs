...
namespace Server.Misc
{
	public class NotorietyHandlers
	{
		...
		public static bool Mobile_AllowHarmful( Mobile from, Mobile target )
		{
			if( from == null || target == null || from.AccessLevel > AccessLevel.Player || target.AccessLevel > AccessLevel.Player )
				return true;
            //Console.WriteLine("nie powinien 111");
			Map map = from.Map;
            //Console.WriteLine("nie powinien 222");
            if (target is PlayerMobile)
            {
                DateTime Shield = ((PlayerMobile)target).m_ADShieldTime;
                Shield = Shield.AddMinutes(PlayerMobile.c_ADShield);
                //Shield.CompareTo(DateTime.Now) < 0
                if (!from.Player &&  Shield.CompareTo(DateTime.Now) > 0)
                {
                    //Console.WriteLine("nie powinien bic prawda_________________________________________________________________________");
                    return false;
                }
            }
            if (from is PlayerMobile)
            {
                ...
                Shield = Shield.AddMinutes(PlayerMobile.c_ADShield);
                if (target is BaseCreature && Shield.CompareTo(DateTime.Now) > 0)
                    return false; // nie mozna zabic moba jak sie ma oslonke
            }
            //Console.WriteLine("nie powinien 33c");
			if( map != null && (map.Rules & MapRules.HarmfulRestrictions) == 0 )
				return true; // In felucca, anything goes

			BaseCreature bc = from as BaseCreature;
            //Console.WriteLine("co mam kurna robic");
			...
			if( !from.Player && !(bc != null && bc.GetMaster() != null && bc.GetMaster().AccessLevel == AccessLevel.Player ) )
			{
				if( !CheckAggressor( from.Aggressors, target ) && !CheckAggressed( from.Aggressed, target ) && target is PlayerMobile && ((PlayerMobile)target).CheckYoungProtection( from ) )
					return false;
                //oslonka?
                //Console.WriteLine("tu kogos bij ???");
		...
		public static int MobileNotoriety( Mobile source, Mobile target )
		{
			...
			if( source.Player && !target.Player && source is PlayerMobile && target is BaseCreature )
			{
				...
				if ( master != null && master.AccessLevel > AccessLevel.Player )
					return Notoriety.CanBeAttacked;
                //Console.Write("moze sie cos tutaj dzieje ?");
                //source.SendAsciiMessage("czesc, to ja sie na ciebie patrze, glabie");
