	...
	public class Profile
	{
		...
		public static void EventSink_ProfileRequest( ProfileRequestEventArgs e )
		{
			...
			if ( beholder.Map != beheld.Map || !beholder.InRange( beheld, 12 ) || !beholder.CanSee( beheld ) )
				return;
            string oldname = beheld.Name;
            beheld.Name = beheld.GetNameUseBy(beholder);
		...
		private static string GetAccountDuration( Mobile m )
		{
			...
			if ( Format( ts.TotalDays, "Jestes z nami juz {0} dni{1}.", out v ) )
				return v;

			if ( Format( ts.TotalHours, "Jestes z nami juz {0} godzin{1}.", out v ) )
				return v;

			if ( Format( ts.TotalMinutes, "Jestes z nami juz {0} minut{1}.", out v ) )
				return v;

			if ( Format( ts.TotalSeconds, "Jestes z nami juz {0} sekund{1}.", out v ) )
				return v;
