...
namespace Server.Mobiles
{
	...
	{
		private const double c_DeathTime = 15.0;
        public const double c_ADShield = 0.5; //after Death shield
		
		//[CommandProperty( AccessLevel.GameMaster )]
		public ArrayList ListaPD = new ArrayList();
		public int PDcount;
        ...
        //SYSTEM IMION
        private List<int> m_KnewNames;

        [CommandProperty(AccessLevel.GameMaster)]
        public List<int> KnewNames
        {
            get { return m_KnewNames; }
            set { m_KnewNames = value; }

        }
        private Dictionary<int, string> m_SetNames;

        [CommandProperty(AccessLevel.GameMaster)]
        public Dictionary<int, string> SetNames
        {
            get { return m_SetNames; }
            set { m_SetNames = value; }

        }

		private DateTime m_DeathTime;
		...
		private class DeathTimer : Timer
		{
			...
			public DeathTimer( Mobile mobile, double time )
				: base( TimeSpan.FromMinutes( time ), TimeSpan.FromMinutes( time ) )
			{
				m_Mobile = mobile;
			...
			protected override void OnTick()
			{
				if(m_Mobile.NetState == null){
					Console.Write("Character offline. Can't resurrect");
					return;
				}
                if (m_Mobile.Alive)
                {
                ...
                {
                    Console.Write("Character online. Resurrecting");
                    m_Mobile.Resurrect();
                }
			}
		}
        public DateTime m_ADShieldTime;
        public Timer m_ADShieldTimer;

        private class ADShieldTimer : Timer
        {
            private Mobile m_Mobile;

            public ADShieldTimer(Mobile mobile, double time)
                : base(TimeSpan.FromMinutes(time), TimeSpan.FromMinutes(time))
            {
                m_Mobile = mobile;
            ...
            protected override void OnTick()
            {
                if (m_Mobile.NetState == null)
                {
                    Console.Write("Character offline.");
                    return;
                }
                if (m_Mobile.Alive)
                {

                    m_Mobile.SendAsciiMessage("Koniec oslonki");
                    m_Mobile.StopTimer();
                    //m_ADShieldTimer.Stop();
                    //if (m_ADShieldTimer != null)
                     //   m_ADShieldTimer.Stop();

                    //m_ADShieldTimer = null;

                    //Console.Write("Character online.");
                    //m_Mobile.SendAsciiMessage("Koniec oslonki");
                    //if (m_Mobile.ADShieldTimer != null)
                   // {
                    //    m_Mobile.ADShieldTimer.Stop();
                    //    m_Mobile.ADShieldTimer = null;
                   // };
                    //m_Mobile.Resurrect();
                    //return;
                }
        ...
        {
            Mobile from = this;
            if (((PlayerMobile)from).m_ADShieldTimer != null)
                  ((PlayerMobile)from).m_ADShieldTimer.Stop();

            ((PlayerMobile)from).m_ADShieldTimer = null;
            //return;
        }
		/*
		private Hashtable  m_SetNames;

		[CommandProperty( AccessLevel.GameMaster )]
		public Hashtable SetNames
		{ 
			get { return m_SetNames; } 
			set	{ m_SetNames = value; } 
		}*/	
		...
		*/
		...
		private Dictionary<int,int> m_LanguageHues;
		...
		public Dictionary<int,int> LanguageHues{
			get { 
				if(m_LanguageHues == null)
					m_LanguageHues = Server.Language.GetDefaultLanguageHues();
				return m_LanguageHues;
			}
		...
		public override int MaxWeight { get { return ((int)((( 40) + (int)(3.5 * this.Str)) * Race.GetWeightModifier())); } }
		...
		public override void ComputeBaseLightLevels( out int global, out int personal )
		{
			...
			bool racialNightSight = (Core.ML && Race.HasRacialNightSight());
		...
		private static void OnLogin( LoginEventArgs e )
		{
			...
			/*string url = "http://strzaly.pl/bahama3";
		...
		/*	((PlayerMobile)from).SetNames = new Hashtable();
			((PlayerMobile)from).SetNames.Add(0, "zero");
			((PlayerMobile)from).SetNames.Add(1, "jeden");
			from.SendAsciiMessage("SetNames[0]: " + ((PlayerMobile)from).SetNames[0]);
			from.SendAsciiMessage("SetNames[1]: " + ((PlayerMobile)from).SetNames[1]);*/

            //SYSTEM IMION 
            //Przeciw crashowi
            if (((PlayerMobile)from).SetNames == null)
            {
                ((PlayerMobile)from).SetNames = new Dictionary<int, string>();
                ((PlayerMobile)from).SetNames.Add(from.Serial, from.Name);
            }
            if (((PlayerMobile)from).KnewNames == null)
            {
                ((PlayerMobile)from).KnewNames = new List<int>();
                ((PlayerMobile)from).KnewNames.Add(from.Serial);
            }
            //KONIEC
            //Zmieniono system skilli i zeby osoby ktore juz skille posiadaja dostaly skill.capa = 100 to trzeba tutaj
            //dodac, zeby wszystkim sie logujacym zmienialo skills[i].cap ==100. 
            //for (int j = 0; j < 52; j++)
            //{
            //    ((PlayerMobile)from).Skills[j].Cap = 100;
            //}
			...
			from.SendAsciiMessage("Rasa: " + from.Race + ", Race0: " + Race.Races[0]);
			if(from.Race == Race.Races[0] || from.Race == Race.Races[1])
				from.SendGump(new RaceGump(from, from));

            if (from.Skills.Total < 100 && from.Race != Race.Races[0])// && from.Race != Race.Races[1])
            {
                int i =1;
                from.SendGump(new WrodzoneGump(from, from));
                //from.SendGump(new WrodzoneGump(from, from));
            }
            // x,y,z w ktorym sie pojawia nowy gracz 869, 3204, -78
            // trzeba porownac i sprawdzic, czy tam siedzi a nie ma gumpa z wyborem miasta
            if ((from.X == 869 && from.Y == 3204 && from.Z == -78 && from.Skills.Total > 100) || (from.X == 869 && from.Y == 3204 && from.Z == -78 && from.Race != Race.Races[0] ))
                from.SendGump(new MiastoStartoweGump(from, from));

            //Timer od oslonki po wskrzeszeniu
            //DateTime ADShield = reader.ReadDateTime();
            DateTime Shield = ((PlayerMobile)from).m_ADShieldTime;
            Shield = Shield.AddMinutes(PlayerMobile.c_ADShield);
            if (Shield.CompareTo(DateTime.Now) < 0)
            {
                //from.Resurrect();
                from.SendAsciiMessage("Nie masz juz oslonki");
            }
            else
            {
                TimeSpan Minutes = Shield.Subtract(DateTime.Now);
                from.SendAsciiMessage("Twoja os³onka skonczy sie za " + Minutes.Minutes + " minut i " + Minutes.Seconds + " sekund.");
                ((PlayerMobile)from).m_ADShieldTimer = new ADShieldTimer(from, Minutes.TotalMinutes);
            }
			if(!from.Alive){

				DateTime ResTime = ((PlayerMobile)from).m_DeathTime;
				...
				if(ResTime.CompareTo(DateTime.Now) < 0){
					...
					from.SendAsciiMessage("Jestes nieprzytomny. Ockniesz sie za " + Minutes.Minutes + " minut i " + Minutes.Seconds + " sekund.");
					((PlayerMobile)from).m_DeathTimer = new DeathTimer(from, Minutes.TotalMinutes);
				}
			}
		}
        ...
        //SYSTEM IMION
        //GetNameUseBy <- znajduje sie jeszcze w Mobiles.cs, wyrzuca DefaultName jesli pmob nie poznaje drugiego pmoba
        //DefaultName <- wyrzuca albo "male", albo "female"
        //TODO: Sprawdza czy zna rase/czy dal osobie jakis Nick w DefaultName trzeba dodac rozroznienie dla Elf i Human
        public override string GetNameUseBy(Mobile m_From)
        {
            Mobile m_Target = (Mobile)this;
            string value = "";
            if (m_From.AccessLevel > AccessLevel.Player)
                return m_Target.Name+" ("+m_Target.Race+")";
            if (m_From == m_Target)
            {
                return m_From.Name + " (" + m_From.Race + ")";
            }
            else if (m_From != m_Target)
            {
                if (((PlayerMobile)m_Target).KnewNames == null)
                {
                    return DefaultName(m_From);
                }
                if (((PlayerMobile)m_Target).KnewNames.Contains(m_From.Serial) == true)
                {
                    //m_Target.SendMessage("Widze go i poznaje");
                    return m_Target.Name + " (" + m_Target.Race + ")"; 
                }
                else
                {
                    //System imion SETNAMES
                    if (((PlayerMobile)m_Target).SetNames == null)
                    {
                        return DefaultName(m_Target);
                    }
                    if (((PlayerMobile)m_Target).SetNames.ContainsKey(m_From.Serial) == false)
                    {
                        //m_Target.SendMessage("Widze go ale nie nadalem mu przydomka");
                        return DefaultName(m_Target);
                    ...
                    {
                        //string value = "";
                        ((PlayerMobile)m_Target).SetNames.TryGetValue(m_From.Serial, out value);
                        //m_Target.SendMessage("Widze go i poznaje, juz mu nadalem przydomek");
                        return value + " (" + DefaultName(m_Target) + ")";
                    }
                    //system imion SETNAMES
                }
            }
		...
		public override void OnDeath( Container c )
		{
			...
			//this.Frozen = true;
			this.m_DeathTimer = new DeathTimer(this, c_DeathTime);
            ...
            //int z = Map.GetAverageZ(this.X, this.Y) - 50;
            //this.SendAsciiMessage("x: {0} , y: {1}, z: {2}", this.X, this.Y, this.Z);
            //if (z < -100) z = -100;
            //this.Z = z;
            //przenosi do mandosa
            this.X = 284;
            this.Y = 3266;
            this.Z = -78;
            //this.SendAsciiMessage("a to na koncu OnDeath . . .x: {0} , y: {1}, z: {2}", this.X, this.Y, this.Z);
           // this.MoveToWorld(new Point3D(4480, 1278, 5), Map.Trammel);
		}
		...
		public override int GetLanguageHue(int languageIndex, int actualHue, MessageType type){
            switch (type)
            {
            ...
            {
                case 1:
                    return 1170; //zmieniciiiiiic
                case 2:
                    return 1173; //szarozielony
                case 3:
                    return 1160; //zmienic
                case 4:
                    return 1161;//ciemnozolty 
                case 5:
                    return 367; // mocny zielony
                case 6:
                    return 287; // jasnoniebieski - blekitny-morski
                case 7:
                    return 1173;//142; // pomarancza - zostawic
                case 8:
                    return 1174; // szary
                case 9:
                    return 137;//1172; //zmienic
                case 10:
                    return 237; //ciemny - zmienic
            }
			return 	LanguageHues[languageIndex];
		}
		...
		public override void Serialize( GenericWriter writer )
		{
            ...
            if (this.m_SetNames != null)
            {
                //writer.Write(this.m_SetNames.Count);
                //Console.Write("array size: " + this.m_SetNames.Count + "\n");
                writer.Write((int)m_SetNames.Count);
                foreach (KeyValuePair<int, string> kvp in SetNames)
                {
            ...
            //oslonka po wskrzeszeniu
            writer.Write((DateTime)this.m_ADShieldTime);
            			writer.Write(LanguageHues.Count);
			foreach(KeyValuePair<int,int> entry in LanguageHues){
				writer.Write(entry.Key);
				writer.Write(entry.Value);
			}
            //SYSTEM IMION  
            //Zapisywanie danych z knewnames 
            if (this.m_KnewNames != null)
            {
                writer.Write(this.m_KnewNames.Count);
                //Console.Write("array size: " + this.m_KnewNames.Count + "\n");
                foreach (int serial in this.m_KnewNames)
                {
            ...
            //SYSTEM IMION  <-KONIEC
			writer.Write(this.PDcount);
			writer.Write(this.ListaPD.Count);
			Console.Write("array size: " + this.ListaPD.Count + "\n");
			foreach(object[] entry in this.ListaPD){
			
				Console.Write("entry0: " + (int)entry[0] + "\n");
				writer.Write((int)entry[0]);
				Console.Write("entry1: " + (string)entry[1] + "\n");
				writer.Write((string)entry[1]);
				Console.Write("entry2: " + (string)entry[2] + "\n");
				writer.Write((string)entry[2]);
			}
			
			writer.Write( (DateTime) this.m_DeathTime );
		...
		public override string ApplyNameSuffix( string suffix )
		{
            ...
            /*if (this.Race == Race.Races[0] || this.Race == Race.Races[1])
            {
                if (suffix.Length == 0)
                    suffix = "("+this.Race+")";
                else
                    suffix = String.Concat(suffix, "(" + this.Race + ")");
            }*/
