...
namespace Server.Engines.Craft
{
	public class CraftGump : Gump
	{
		...
		private const int FontColor = 0x000000;
		...
		private CraftGump( Mobile from, CraftSystem craftSystem, BaseTool tool, object notice, CraftPage page ) : base( 40, 40 )
		{
			...
			AddBackground( 0, 0, 530, 437, 9300 );
			AddImageTiled( 10, 10, 510, 22, 1045 );
			AddImageTiled( 10, 292, 150, 45, 1048 );
			AddImageTiled( 165, 292, 355, 45, 1049 );
			AddImageTiled( 10, 342, 510, 85, 1050 );
			AddImageTiled( 10, 37, 200, 250, 1046 );
			AddImageTiled( 215, 37, 305, 250, 1047 );
			/* AddAlphaRegion( 10, 10, 510, 417 ); */

			if ( craftSystem.GumpTitleNumber > 0 )
				AddHtmlLocalized( 10, 12, 510, 20, craftSystem.GumpTitleNumber, 0, false, false );
			else
				AddHtml( 10, 12, 510, 20, craftSystem.GumpTitleString, false, false );

			AddHtmlLocalized( 10, 37, 200, 22, 1044010, 0, false, false ); // <CENTER>CATEGORIES</CENTER>
			AddHtmlLocalized( 215, 37, 305, 22, 1044011, 0, false, false ); // <CENTER>SELECTIONS</CENTER>
			AddHtmlLocalized( 10, 302, 150, 25, 1044012, 0, false, false ); // <CENTER>NOTICES</CENTER>

			AddButton( 15, 402, 5052, 5053, 0, GumpButtonType.Reply, 0 );
			AddHtmlLocalized( 50, 405, 150, 18, 1011441, 0, false, false ); // EXIT

			AddButton( 270, 402, 5050, 5051, GetButtonID( 6, 2 ), GumpButtonType.Reply, 0 );
			AddHtmlLocalized( 305, 405, 150, 18, 1044013, 0, false, false ); // MAKE LAST

			// Mark option
			if ( craftSystem.MarkOption )
			{
				AddButton( 270, 362, 5050, 5051, GetButtonID( 6, 6 ), GumpButtonType.Reply, 0 );
				AddHtmlLocalized( 305, 365, 150, 18, 1044017 + (context == null ? 0 : (int)context.MarkOption), 0, false, false ); // MARK ITEM
			}
			// ****************************************
			...
			{
				AddButton( 15, 342, 5050, 5051, GetButtonID( 6, 1 ), GumpButtonType.Reply, 0 );
				AddHtmlLocalized( 50, 345, 150, 18, 1044259, 0, false, false ); // SMELT ITEM
			}
			// ****************************************
			...
			{
				AddButton( 270, 342, 5050, 5051, GetButtonID( 6, 5 ), GumpButtonType.Reply, 0 );
				AddHtmlLocalized( 305, 345, 150, 18, 1044260, 0, false, false ); // REPAIR ITEM
			}
			// ****************************************
			...
			{
				AddButton( 270, 382, 5050, 5051, GetButtonID( 6, 8 ), GumpButtonType.Reply, 0 );
				AddHtmlLocalized( 305, 385, 150, 18, 1061001, 0, false, false ); // ENHANCE ITEM
			}
			// ****************************************

			if ( notice is int && (int)notice > 0 )
				AddHtmlLocalized( 170, 295, 350, 40, (int)notice, 0, false, false );
			...
			if ( craftSystem.CraftSubRes.Init )
			{
				...
				AddButton( 15, 362, 5050, 5051, GetButtonID( 6, 0 ), GumpButtonType.Reply, 0 );

				if ( nameNumber > 0 )
					AddHtmlLocalized( 50, 365, 250, 18, nameNumber, resourceCount.ToString(), 0, false, false );
				else
			...
			if ( craftSystem.CraftSubRes2.Init )
			{
				...
				AddButton( 15, 382, 5050, 5051, GetButtonID( 6, 7 ), GumpButtonType.Reply, 0 );

				if ( nameNumber > 0 )
                    AddHtmlLocalized( 50, 385, 250, 18, nameNumber, resourceCount.ToString(), 0, false, false );
				else
		...
		public void CreateResList( bool opt, Mobile from )
		{
			...
			for ( int i = 0; i < res.Count; ++i )
			{
				...
				{
					if ( i > 0 )
						AddButton( 485, 260, 5050, 5051, 0, GumpButtonType.Page, (i / 10) + 1 );

					AddPage( (i / 10) + 1 );

					if ( i > 0 )
						AddButton( 455, 260, 5052, 5053, 0, GumpButtonType.Page, i / 10 );

					CraftContext context = m_CraftSystem.GetContext( m_From );

					AddButton( 220, 260, 5050, 5051, GetButtonID( 6, 4 ), GumpButtonType.Reply, 0 );
					AddHtmlLocalized( 255, 263, 200, 18, (context == null || !context.DoNotColor) ? 1061591 : 1061590, 0, false, false );
				}
				...
				AddButton( 220, 60 + (index * 20), 5050, 5051, GetButtonID( 5, i ), GumpButtonType.Reply, 0 );

				if ( subResource.NameNumber > 0 )
					AddHtmlLocalized( 255, 63 + (index * 20), 250, 18, subResource.NameNumber, resourceCount.ToString(), 0, false, false );
				else
			...
			if ( items.Count > 0 )
			{
				for ( int i = 0; i < items.Count; ++i )
				{
					...
					if ( index == 0 )
					{
						if ( i > 0 )
						{
							AddButton( 370, 260, 5050, 5051, 0, GumpButtonType.Page, (i / 10) + 1 );
							AddHtmlLocalized( 405, 263, 100, 18, 1044045, 0, false, false ); // NEXT PAGE
						}
						...
						if ( i > 0 )
						{
							AddButton( 220, 260, 5052, 5053, 0, GumpButtonType.Page, i / 10 );
							AddHtmlLocalized( 255, 263, 100, 18, 1044044, 0, false, false ); // PREV PAGE
						}
					}

					AddButton( 220, 60 + (index * 20), 5050, 5051, GetButtonID( 3, i ), GumpButtonType.Reply, 0 );

					if ( craftItem.NameNumber > 0 )
						AddHtmlLocalized( 255, 63 + (index * 20), 220, 18, craftItem.NameNumber, 0, false, false );
					else
						AddLabel( 255, 60 + (index * 20), LabelHue, craftItem.NameString );

					AddButton( 480, 60 + (index * 20), 1051, 1052, GetButtonID( 4, i ), GumpButtonType.Reply, 0 );
				}
			}
			else
			{
				...
				AddHtmlLocalized( 230, 62, 200, 22, 1044165, 0, false, false ); // You haven't made anything yet.
			}
		}
		...
		public void CreateItemList( int selectedGroup )
		{
			...
			for ( int i = 0; i < craftItemCol.Count; ++i )
			{
				...
				AddButton( 220, 60 + (index * 20), 5050, 5051, GetButtonID( 1, i ), GumpButtonType.Reply, 0 );
				...
				AddButton( 480, 60 + (index * 20), 1051, 1052, GetButtonID( 2, i ), GumpButtonType.Reply, 0 );
			}
		}
		...
		public int CreateGroupList()
		{
			...
			AddButton( 15, 60, 5050, 5051, GetButtonID( 6, 3 ), GumpButtonType.Reply, 0 );
			AddHtmlLocalized( 50, 63, 150, 18, 1044014, 0, false, false ); // LAST TEN

			for ( int i = 0; i < craftGroupCol.Count; i++ )
			{
				CraftGroup craftGroup = craftGroupCol.GetAt( i );

				AddButton( 15, 80 + (i * 20), 5050, 5051, GetButtonID( 0, i ), GumpButtonType.Reply, 0 );

				if ( craftGroup.NameNumber > 0 )
					AddHtmlLocalized( 50, 83 + (i * 20), 150, 18, craftGroup.NameNumber, 0, false, false );
				else
