...
namespace Server.Engines.Craft
{
	public class CraftGumpItem : Gump
	{
		...
		public CraftGumpItem( Mobile from, CraftSystem craftSystem, CraftItem craftItem, BaseTool tool ) : base( 40, 40 )
		{
			...
			AddPage( 0 );
			AddBackground( 0, 0, 530, 417, 9300 );
			AddImageTiled( 10, 10, 510, 22, 1042 );
			AddImageTiled( 10, 37, 150, 148, 1044 );
			AddImageTiled( 165, 37, 355, 90, 1043 );
			AddImageTiled( 10, 190, 155, 22, 1041 );
			AddImageTiled( 10, 217, 150, 53, 1040 );
			AddImageTiled( 165, 132, 355, 80, 1028 );
			AddImageTiled( 10, 275, 155, 22, 1041 );
			AddImageTiled( 10, 302, 150, 53, 1040 );
			AddImageTiled( 165, 217, 355, 80, 1028 );
			AddImageTiled( 10, 360, 155, 22, 1041 );
			AddImageTiled( 165, 302, 355, 80, 1028 );
			AddImageTiled( 10, 387, 510, 22, 1042 );
		    /* AddAlphaRegion( 10, 10, 510, 399 ); */

			AddHtmlLocalized( 170, 40, 150, 20, 1044053, 0, false, false ); // ITEM
			AddHtmlLocalized( 10, 192, 150, 22, 1044054, 0, false, false ); // <CENTER>SKILLS</CENTER>
			AddHtmlLocalized( 10, 277, 150, 22, 1044055, 0, false, false ); // <CENTER>MATERIALS</CENTER>
			AddHtmlLocalized( 10, 362, 150, 22, 1044056, 0, false, false ); // <CENTER>OTHER</CENTER>
			...
			AddButton( 15, 387, 5052, 5253, 0, GumpButtonType.Reply, 0 );
			AddHtmlLocalized( 50, 390, 150, 18, 1044150, 0, false, false ); // BACK

			bool needsRecipe = ( craftItem.Recipe != null && from is PlayerMobile && !((PlayerMobile)from).HasRecipe( craftItem.Recipe ) );

			if( needsRecipe )
			{
				AddButton( 270, 387, 5050, 5051, 0, GumpButtonType.Page, 0 );
				AddHtmlLocalized( 305, 390, 150, 18, 1044151, GreyLabelColor, false, false ); // MAKE NOW
			}
			else
			{
				AddButton( 270, 387, 5050, 5051, 1, GumpButtonType.Reply, 0 );
				AddHtmlLocalized( 305, 390, 150, 18, 1044151, 0, false, false ); // MAKE NOW
			}

			if ( craftItem.NameNumber > 0 )
				AddHtmlLocalized( 330, 40, 180, 18, craftItem.NameNumber, 0, false, false );
			else
				AddLabel( 330, 40, LabelHue, craftItem.NameString );

			if ( craftItem.UseAllRes )
				AddHtmlLocalized( 170, 302 + (m_OtherCount++ * 20), 310, 18, 1048176, 0, false, false ); // Makes as many as possible at once
			...
			if( craftItem.RequiresSE )
				AddHtmlLocalized( 170, 302 + (m_OtherCount++ * 20), 310, 18, 1063363, 0, false, false ); //* Requires the "Samurai Empire" expansion
			 * */
			...
			if( craftItem.RequiredExpansion != Expansion.None )
			{
				...
				TextDefinition.AddHtmlText( this, 170, 302 + (m_OtherCount++ * 20), 310, 18, RequiredExpansionMessage( craftItem.RequiredExpansion ), false, false, supportsEx ? 0 : RedLabelColor, supportsEx ? LabelHue : RedLabelHue );
			}
		...
		public void DrawItem()
		{
			...
			{
				AddHtmlLocalized( 170, 302 + (m_OtherCount++ * 20), 310, 18, 1044059, 0, false, false ); // This item may hold its maker's mark
		...
		public void DrawSkill()
		{
			for ( int i = 0; i < m_CraftItem.Skills.Count; i++ )
			{
				...
				AddHtmlLocalized( 170, 132 + (i * 20), 200, 18, 1044060 + (int)skill.SkillToMake, 0, false, false );
			...
			AddHtmlLocalized( 170, 80, 250, 18, 1044057, 0, false, false ); // Success Chance:
		...
		public void DrawResource()
		{
			...
			for ( int i = 0; i < m_CraftItem.Resources.Count - (cropScroll ? 1 : 0) && i < 4; i++ )
			{
				...
				{
					retainedColor = true;
					AddHtmlLocalized( 170, 302 + (m_OtherCount++ * 20), 310, 18, 1044152, 0, false, false ); // * The item retains the color of this material
					AddLabel( 500, 219 + (i * 20), LabelHue, "*" );
				}

				if ( nameNumber > 0 )
					AddHtmlLocalized( 170, 219 + (i * 20), 310, 18, nameNumber, 0, false, false );
				else
			...
			{
				AddHtmlLocalized( 170, 219 + (m_CraftItem.Resources.Count * 20), 310, 18, 1044447, 0, false, false );
				AddLabel( 430, 219 + (m_CraftItem.Resources.Count * 20), LabelHue, "1" );
			}

			if ( cropScroll )
				AddHtmlLocalized( 170, 302 + (m_OtherCount++ * 20), 360, 18, 1044379, 0, false, false ); // Inscribing scrolls also requires a blank scroll and mana.
		}
