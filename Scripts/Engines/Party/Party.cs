		...
		public void Add( Mobile m )
		{
			...
			if ( mi == null )
			{
				...
				for ( int i = 0; i < m_Members.Count; ++i )
				{
					...
					if ( f != m )
					{
                        f.Send(new MobileStatusCompact(m.CanBeRenamedBy(f), m, f));
						f.Send( attrs );
                        m.Send(new MobileStatusCompact(f.CanBeRenamedBy(m), f, m));
		...
		public void OnStatsQuery( Mobile beholder, Mobile beheld )
		{
			if ( beholder != beheld && Contains( beholder ) && beholder.Map == beheld.Map && Utility.InUpdateRange( beholder, beheld ) )
			{
				if ( !beholder.CanSee( beheld ) )
                    beholder.Send(new MobileStatusCompact(beheld.CanBeRenamedBy(beholder), beheld, beholder));
