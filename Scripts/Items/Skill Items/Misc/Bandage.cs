...
namespace Server.Items
{
	public class Bandage : Item, IDyable
	{
		...
		private class InternalTarget : Target
		{
			...
			protected override void OnTarget( Mobile from, object targeted )
			{
                ...
                //dodawanie, aby mozna bylo resnac mobila po smierci w nowym systemie
               // Corpse corpse = item as Corpse;
                ...
                else if (targeted is Corpse)
                {
                    if (from.InRange(m_Bandage.GetWorldLocation(), Bandage.Range))
                    {
                        //if (targeted.Owner is PlayerMobile)
                        //{
                        Corpse corpse = targeted as Corpse;
                        //from.SendAsciiMessage("To on jest wlascicielem tego ciala: {0}", corpse.Owner.Name);
                            if (BandageContext.BeginHeal(from, corpse.Owner) != null)
                            {
                                m_Bandage.Consume();
                            }
                        //}
	...
	public class BandageContext
	{
		...
		public void EndHeal()
		{
            ...
            else if (!m_Healer.InRange(m_Patient, Bandage.Range) && m_Patient.Alive)
			{
